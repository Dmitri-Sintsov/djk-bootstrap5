                ...
                for (var i = 0; i < elem.attributes.length; i++) {
                    ...
                    if (name !== attrName) {
                        if (name.substr(0, 3) === 'bs-') {
                            elem.setAttribute(
                                'data-bs-' + name.substr(3), elem.attributes[i].value
                            );
...
function UiPopover($elem) {
    ...
    UiPopover.create = function(options) {
        ...
        if (typeof options.template === 'string') {
            // Convert universal .bs-popover-body to bs5 .popover-body
...
function UiTooltip($elem) {
    ...
    UiTooltip.init = function($elem) {
        ...
        this.tooltip = bootstrap.Tooltip.getInstance(this.elem);
    };
    ...
    UiTooltip.create = function(options) {
        ...
        this.tooltip = new bootstrap.Tooltip(this.elem, options);
    };
...
/**
 * Uses Tempus Dominus for Bootstrap, version 5.1.2.
 */
